...
app.get('/api/farm/:farmId/sensor-data', async (req, res) => {
  ...
  const query = 'SELECT temperature, humidity, light, soilPH, time FROM sensor_data WHERE farm_id = ? ORDER BY created_at DESC LIMIT 1';
...
app.post('/api/farm', upload.single('image'), async (req, res) => {
  const { name, location, latitude, longitude, farmer_first_name, farmer_last_name } = req.body;
  const imageUrl = req.file ? `/uploads/${req.file.filename}` : null; // Construct the image URL

  console.log("Received farm data:", { name, location, latitude, longitude, farmer_first_name, farmer_last_name, imageUrl }); // Log the received data
  ...
  try {
    // Fetch the farmer's ID based on their first name and last name
    const farmerQuery = 'SELECT id FROM users WHERE first_name = ? AND last_name = ? AND role = "farmer"';
    db.query(farmerQuery, [farmer_first_name, farmer_last_name], async (err, results) => {
      if (err) {
      ...
      const farmer_id = results[0].id; // Get the farmer's ID
      ...
      const sql = 'INSERT INTO farms (name, owner_id, location, latitude, longitude, image_url, weather) VALUES (?, ?, ?, ?, ?, ?, ?)';
      db.query(
        sql,
        ...
        (err, result) => {
          if (err) {
            ...
            return res.status(500).json({ message: 'Error adding farm' });
          }
